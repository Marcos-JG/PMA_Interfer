...
export const sendVerificationEmail = async (email, name, verificationToken) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: 'Verifica tu dirección de email - Interfer',
      html: `
...
export const sendPasswordResetEmail = async (email, name, resetToken) => {
  ...
  try {
    ...
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: 'Recuperación de contraseña - Interfer',
      html: `
...
export const sendWelcomeEmail = async (email, name) => {
  ...
  try {
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: '¡Bienvenido a Interfer!',
      html: `
        <h2>¡Bienvenido a Interfer, ${name}!</h2>
...
export const sendPasswordChangedEmail = async (email, name) => {
  ...
  try {
    const mailOptions = {
      from: `${config.smtp.fromName} <${config.smtp.fromEmail}>`,
      to: email,
      subject: 'Contraseña Actualizada - Interfer',
      html: `
